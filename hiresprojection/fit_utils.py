...
def get_ellipse_params(v, limb, subpt, wcs, img_size):
    ...
    X[1] = crpix[0] / img_size[1] - 0.5
    X[2] = crpix[1] / img_size[0] - 0.5
    X[3] = alpha / np.pi
    ...
    def func(inp):
        ...
        b = np.matmul(rotation_matrix_plate, dRADec).T / A + np.array([X0, Y0])
    ...
    A, x0, y0, alpha = output.x

    def func_alpha(inp):
        return func([A, x0, y0, inp[0]])
    ...
    wcs.wcs.cdelt = np.asarray([np.abs(A) / RR, np.abs(A)]) * 180. / np.pi
